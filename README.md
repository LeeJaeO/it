...
results.detections에 여러 얼굴이 들어오면,

각 얼굴에 대해 **중심점 근처 깊이(z_meter)**를 구한 뒤,

그 중 z_meter가 가장 작은(카메라에 가장 가까운) 얼굴만 선택해서 이후 로직 적용.

2) 실물 vs 사진 구분 (깊이 변동성 기반)

Azure Kinect의 장점이 깊이(Depth) 이니까, 사진/모니터 같은 평면은:

얼굴 영역의 깊이맵이 거의 평평 → 깊이 표준편차(std)가 작음

반대로, 실제 얼굴은 코/볼/눈 등의 굴곡 때문에

깊이 값이 꽤 요동 → 표준편차가 큼

그래서:

얼굴 박스의 **중앙 부분(내부 40% 정도)**만 잘라서 depth patch를 뽑고
...
std < THRESH 이면 사진(FAKE), 그 이상이면 실물(REAL) 로 판단

예시로 DEPTH_STD_THRESHOLD_MM = 15.0 (15mm) 정도로 두고, 실제 환경에서 튜닝하는 것을 전제로 둡니다.
